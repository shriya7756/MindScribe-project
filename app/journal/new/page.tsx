...
export default function NewJournalPage() {
    ...
    if (files && files.length > 0) {
      Array.from(files).forEach((file) => {
        if (file.size > 50 * 1024 * 1024) {
          alert(`Video ${file.name} is too large. Please select videos under 50MB.`)
          return
        }
        ...
        reader.onerror = () => {
          alert(`Error reading video ${file.name}`)
        }
        reader.readAsDataURL(file)
      })
    }
  ...
  const handleSave = () => {
    if (!title.trim() && !content.trim() && images.length === 0 && videos.length === 0) {
      alert("Please add some content to your journal entry before saving.")
      return
    }
    ...
    try {
      ...
      const existingJournals = JSON.parse(localStorage.getItem("mindscribe_journals") || "[]")
      const updatedJournals = [journal, ...existingJournals]
      ...
      <header className="bg-white/80 backdrop-blur-sm border-b border-yellow-200 sticky top-0 z-10">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-semibold text-amber-800">New Journal Entry</h1>
          </div>
          <Button
            onClick={handleSave}
        ...
        <Card className="max-w-4xl mx-auto border-yellow-200" style={{ backgroundColor: backgroundColor + "40" }}>
          <CardHeader>
            <div className="flex justify-between items-center">
              <CardTitle className="text-amber-800">Create Your Journal</CardTitle>
